<div
    x-data="{ usePoints: @entangle('usePoints').live, hasEnoughPoints: {{ $hasEnoughPoints ? 'true' : 'false' }} }"
...
>
    <div class="flex items-center justify-between mb-3">
        <div>
            ...
            <p class="text-sm text-zinc-500 dark:text-zinc-400">
                Your balance: {{ number_format($pointsBalance) }} points ({{ $pointsValueFormatted }})
            </p>
        </div>
        <div class="flex items-center">
            <input
                ...
                class="form-checkbox h-5 w-5 text-amber-600 rounded border-zinc-300 focus:ring-amber-500"
                x-model="usePoints"
    ...
    <div class="bg-zinc-50 dark:bg-zinc-800 rounded p-3 text-sm" x-show="hasEnoughPoints" x-cloak>
        @if($hasEnoughPoints)
            <div class="flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-green-500 mr-2" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                </svg>
                <span>You have enough points to cover this purchase ({{ number_format($requiredPoints) }} points)</span>
            </div>
        @endif
    </div>
    ...
    <div class="bg-zinc-50 dark:bg-zinc-800 rounded p-3 text-sm" x-show="!hasEnoughPoints" x-cloak>
        <div class="flex items-center">
            ...
            <span>You need {{ number_format($requiredPoints) }} points ({{ $requiredValueFormatted }}) to cover this purchase</span>
        </div>
    </div>
</div>
